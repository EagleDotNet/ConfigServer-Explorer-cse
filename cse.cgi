...
use lib '/usr/local/cyberpanel';
require cyberpanel::Form;
require Whostmgr::ACLS;
require cyberpanel::Template;
require cyberpanel::Rlimit;
require cyberpanel::Version::Tiny;
###############################################################################
# start main
...
%form = cyberpanel::Form::parseform();
...
cyberpanel::Rlimit::set_rlimit_to_infinity();


$script = "cse.cgi";
$images = "cse";
$versionfile = "/usr/local/cyberpanel/whostmgr/docroot/cgi/configserver/cse/cseversion.txt";
...
my $reregister;
my $modalstyle;
if ($cyberpanel::Version::Tiny::major_version >= 65) {
	if (-e "/usr/local/cyberpanel/whostmgr/docroot/cgi/configserver/${thisapp}/${thisapp}.conf") {
		sysopen (my $CONF, "/usr/local/cyberpanel/whostmgr/docroot/cgi/configserver/${thisapp}/${thisapp}.conf", O_RDWR | O_CREAT);
		flock ($CONF, LOCK_EX);
		my @confdata = <$CONF>;
		chomp @confdata;
		...
		if ($reregister) {
			...
			&printcmd("/usr/local/cyberpanel/bin/register_appconfig","/usr/local/cyberpanel/whostmgr/docroot/cgi/configserver/${thisapp}/${thisapp}.conf");
...
print "Content-type: text/html\r\n\r\n";
#if ($cyberpanel::Version::Tiny::major_version < 65) {$modalstyle = "style='top:120px'"}
...
foreach my $key (keys %form) {
	if (($form{$key} =~ /\/tmp\/cyberpanel3upload\./) or ($form{$key} =~ /\/tmp\/cyberpanel\.TMP/) or ($form{$key} =~ /\/tmp\/cyberpanel_Form_file\.upload\./)) {$uploadingfile = 1};
}
...
unless ($form{do} eq "console") {
	...
	print "<p>&copy;2006-2019, <a href='http://www.configserver.com' target='_blank'>ConfigServer Services</a> (Way to the Web Limited)</p>\n";
}
print <<EOF;
<script>
	\$("#loader").hide();
...
unless ($form{do} eq "console") {
	close SCRIPTOUT;
	select STDOUT;
	cyberpanel::Template::process_template(
		'whostmgr',
		{
...
if (-e "/usr/local/cyberpanel/whostmgr/docroot/cgi/csenocheck") {
	open (IN, "<", "/usr/local/cyberpanel/whostmgr/docroot/cgi/csenocheck");
	flock (IN, LOCK_SH);
	my $time = <IN>;
	close (IN);
	chomp $time;
	$retry = time - $time;
	if ($retry > $retrytime) {unlink ("/usr/local/cyberpanel/whostmgr/docroot/cgi/csenocheck")}
}
unless (-e "/usr/local/cyberpanel/whostmgr/docroot/cgi/csenocheck") {
	...
	unless ($up) {
		sysopen (OUT, "/usr/local/cyberpanel/whostmgr/docroot/cgi/csenocheck", O_WRONLY | O_CREAT);
		flock (OUT, LOCK_EX);
		print OUT time;
		close (OUT);
...
my @filebodies;
foreach my $key (keys %form) {
	if (($form{$key} =~ /\/tmp\/cyberpanel3upload\./) or ($form{$key} =~ /\/tmp\/cyberpanel\.TMP/) or ($form{$key} =~ /\/tmp\/cyberpanel_Form_file\.upload\./)) {
		push (@filenames, $key);
...
sub getdownloadserver {
	my @servers;
	my $downloadservers = "/usr/local/cyberpanel/whostmgr/docroot/cgi/configserver/cse/downloadservers";
	my $chosen;
